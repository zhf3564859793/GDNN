...
def main():
    ...
    parser.add_argument('--MLP_hidden_channels', type=int, default=1024)
    ...
    if args.use_sage:
        model = SAGE(args.in_channels, args.hidden_channels,
                     args.out_channels, args.num_layers,
                     args.dropout).to(device)
    else:
        model = GCN(args.in_channels, args.hidden_channels,
                    args.out_channels, args.num_layers,
                    args.dropout).to(device)
    ...
    Node_degree=pd.DataFrame(data=ddiNet.degree,columns=["Node","Degree"])
    Node_degree=Node_degree.sort_values(by=["Degree"],ascending=False)
    Node_degree=Node_degree.reset_index(drop=True)
    print(Node_degree)
    Node_sort_by_degree=np.array(Node_degree['Node'])
    print('Node_degree转换后:',Node_sort_by_degree)
    #节点度最大的512个节点
    Node_degree_512th=Node_sort_by_degree[0:768]
    print('节点度最大的512个节点',Node_degree_512th)
    print(len(Node_degree_512th))
    #节点度最小的512个节点
    Node_degree_512th_last=Node_sort_by_degree[3489:4268]
    print('节点度最小的512个节点',Node_degree_512th_last)

    print('0节点到4节点最短路径: ',nx.shortest_path(ddiNet, source=478, target=478))
    ...
    #随机选取ddi图中512个节点作为靶点
    badian=np.random.choice(4267, 768, replace=False)
    print('badian是:',badian)

    distance_feature=[]

    for x in range(0,4267):
        distance_feature.append([])
        for y in Node_degree_512th:
            dis=distance_encoding(x,y)
            distance_feature[x].append(dis)

    print('distance_feature是:',distance_feature)
    distance_feature=torch.tensor(distance_feature,dtype=torch.float)
    emb=torch.nn.Embedding.from_pretrained(distance_feature,freeze=False)
    #emb1 = torch.nn.Embedding(data.adj_t.size(0),256)
    #emb=torch.cat((emb.weight,emb1.weight) ,dim=1)
    #emb=torch.nn.Embedding.from_pretrained(emb,freeze=False)
                                                  
    emb=emb.to(device)                        
    predictor = LinkPredictor(args.out_channels, args.MLP_hidden_channels, 1,
    ...
    for run in range(args.runs):
        ...
        #plt.title("DE+1024_smallest degree node+2048hidden+lr=0.001+dropout=0.5")
        #plt.xlabel("Epoch")
